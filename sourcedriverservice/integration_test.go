...
func newSourceDriverTestRuntime(t *testing.T, socket string, server *wire.Server) *daemon.Runtime {
	...
	policy, err := trust.NewTrustPolicy(trust.TrustPolicyConfig{
		...
		Roles: map[trust.PeerRole]trust.Requirement{
			"stop":    {TeamID: "DAEMONKITTEST", SigningIdentifier: "com.yasyf.fusekit.sourcedriver.stop"},
			"runtime": {TeamID: "DAEMONKITTEST", SigningIdentifier: "com.yasyf.fusekit.sourcedriver.runtime"},
		},

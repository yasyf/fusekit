...
func swiftShapeValidation(name, indent string) string {
	...
	case "BrokerCommand":
		lines = []string{
			...
			"switch kind {",
			"case .registerDomain: guard registration != nil, domainID == nil, notification == nil, cutover == nil else { throw CatalogProtocolCodingError.invalidShape(\"register_domain command shape\") }",
			"case .removeDomain: guard registration == nil, domainID != nil, notification == nil, cutover == nil else { throw CatalogProtocolCodingError.invalidShape(\"remove_domain command shape\") }",
			"case .listDomains: guard registration == nil, domainID == nil, notification == nil, cutover == nil else { throw CatalogProtocolCodingError.invalidShape(\"list_domains command shape\") }",
			"case .signalDomain: guard registration == nil, domainID == nil, notification != nil, cutover == nil else { throw CatalogProtocolCodingError.invalidShape(\"signal_domain command shape\") }",
			"case .cutoverDomains: guard registration == nil, domainID == nil, notification == nil, cutover != nil else { throw CatalogProtocolCodingError.invalidShape(\"cutover_domains command shape\") }",
			"}",
		}
	}
	if len(lines) == 0 {

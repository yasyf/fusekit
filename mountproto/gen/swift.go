...
func renderSwift() string {
	return fmt.Sprintf(`// Code generated by mountproto/gen; DO NOT EDIT.

import Foundation

public enum MountProtocol {
    public static let version: UInt16 = 1
    public static let runtimeProtocolVersion: UInt16 = %d
...
public struct MountNativeMountProof: Codable, Sendable, Equatable {
    ...
    private func validate() throws {
        ...
        guard presentationRoot.hasPrefix("/"), presentationRoot != "/",
              ...
              source == "fuse-t:/\(leaf)", rootReadEpoch != 0 else {
...
public struct MountRuntimeHealthResponse: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        try mountValidateKeys(
            ...
            allowed: [
                ...
                "native_mount", "native_phase", "process_generation", "protocol", "ready", "readiness_phase",
        ...
        if code != .ok {
            ...
            guard emptyState.allSatisfy(\.isEmpty),
                  ...
                  try container.decodeIfPresent(MountNativeMountProof.self, forKey: .nativeMount) == nil else {
                throw MountProtocolCodingError.invalidShape("failed runtime health response carries health state")
            }
    ...
    private func validate() throws {
        ...
        guard readinessPhase != .ready || readinessStep == .published,
              ...
              readinessPhase != .starting || readinessStep != .published else {
            throw MountProtocolCodingError.invalidShape("runtime readiness phase and step disagree")
        }
        guard draining == (state == .draining),
              draining == (readinessPhase == .draining) else {
            throw MountProtocolCodingError.invalidShape("runtime draining state, flag, and readiness phase disagree")
        }
        ...
        if readinessPhase == .ready {
            guard nativePhase == .live, nativeMount != nil,
                  brokerPhase == .disabled || brokerPhase == .live else {
                throw MountProtocolCodingError.invalidShape("ready runtime lacks exact native or broker readiness")
            }
        }
        let exactReady = !draining && readinessPhase == .ready && readinessStep == .published &&
            nativePhase == .live && nativeMount != nil &&
            (brokerPhase == .disabled || brokerPhase == .live)
        guard ready == exactReady else {
            throw MountProtocolCodingError.invalidShape("runtime ready flag and exact serving prerequisites disagree")
        }
    }
}
`, runtimeProtocolVersion, schemaBuild())
}

...
require_released_dependencies() {
  grep -Eq '^[[:space:]]*github\.com/yasyf/daemonkit v0\.3\.1$' go.mod ||
    fail "go.mod must pin the published daemonkit v0.3.1 tag before FuseKit is tagged"
  grep -Fq '.package(url: "https://github.com/yasyf/daemonkit.git", exact: "0.3.1")' Package.swift ||
    fail "Package.swift must pin the published daemonkit 0.3.1 tag before FuseKit is tagged"
  grep -Fq '"version" : "0.3.1"' Package.resolved ||
    fail "Package.resolved must resolve the published daemonkit 0.3.1 tag"

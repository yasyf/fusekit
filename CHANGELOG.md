...
- Requires daemonkit v0.23.0, which moves every daemon's private state from
  `~/.daemonkit/agents/<Label>` to `~/.daemonkit/a/<Label>`. The v0.22.0 root
  spent 18 of the 103 bytes darwin's `sun_path` leaves a socket path, which put
  cc-orchestrate's pty daemons at 104 under a long account name — one byte
  over, and `paths.Socket` refused every spawn. The byte comes out of the root
  rather than out of any consumer's label.

  A holder comes up fresh under the new root. There is no migration and no
  fallback read, so `~/.daemonkit/agents/<Label>` is left where it lies, to be
  deleted by hand once every consumer has shipped. The Swift package moves in
  the same release — `Package.swift` pins DaemonKit at `exact: "0.23.0"` — so a
  Swift consumer of FuseKit must be on daemonkit 0.23.0 as well; a package
  graph still asking for 0.22.0 has no solution.
...
- **Helper installs survive a sandboxed caller environment.** FuseKit now pins
  daemonkit 0.20.9 across Go and Swift, so every user-home-derived durable path
  — the launchd plist directory, the stable program root, and the artifact
  store — resolves the invoking user's home through the passwd database instead
  of `$HOME`. A helper installed from Homebrew's postinstall, which runs under a
  temporary sandboxed `HOME`, no longer stages plists and binaries into a
  throwaway directory that launchd refuses to bootstrap from.

- **A refused `launchctl` bootstrap fails instead of being waited out.** Exit 5
  is no longer classified as a transient in-flux state, so a permanent launchd
  denial surfaces with the bootstrapped plist path and a pointer at launchd's
  own log rather than burning the retry budget.

- **A failed desired agent no longer wedges the install that would fix it.**
  Controller startup recovery reconciles in a recovery mode: a persisted desired
  agent whose install fails is left as drift for a later converge to retry
  instead of failing recovery closed. Caller-requested convergence stays strict.
...
- **Go and Swift pin the same exact daemonkit release.** `Package.swift` and
  `Package.resolved` now pin daemonkit 0.20.6, matching the `go.mod` pin. A
  Swift consumer resolved at 0.20.6 no longer meets an unsatisfiable package
  graph through FuseKit's exact 0.19.1 requirement.
...
- **Helper entitlement attestations use daemonkit's canonical plist digest.**
  FuseKit now records the same complete semantic entitlement dictionary digest
  that daemonkit verifies, while retaining exact forbidden-key checks.

## [1.15.0] - 2026-07-24

### Changed

- **Clean helper installs use one canonical candidate-planning surface.**
  Consumers bind a packaged signed application to FuseKit's fixed
  `~/Applications` service policy before the target exists, while installed
  deployment and runtime planning remain strict. The redundant plan-instance
  candidate methods are removed.

## [1.14.0] - 2026-07-24

### Changed

- **Daemon trust verification is proven before every runtime serves.** FuseKit
  now pins daemonkit 0.18.0 across Go and Swift, gives direct worker-runtime
  claims daemonkit's dedicated verifier budgets, and dispatches the verifier
  child in every runtime-hosting executable. A broken product entrypoint now
  fails runtime startup instead of silently rejecting every peer.
...
- **DaemonKit is pinned exactly at 0.9.0 across Go and Swift.** The hard cut
  adopts daemonkit's exact v1 durable process, service, and synchronization
  schemas; older persisted runtime state is rejected rather than migrated or
...
- **The macOS FUSE callback gate runs only FUSE-tagged test packages.** Tagged
  ...
  callback test step discovers directories containing FUSE-tagged tests instead
  of redundantly running the long catalog suite under an unrelated build tag.
...
[1.15.0]: https://github.com/yasyf/fusekit/compare/v1.14.0...v1.15.0
[1.14.0]: https://github.com/yasyf/fusekit/compare/v1.13.3...v1.14.0

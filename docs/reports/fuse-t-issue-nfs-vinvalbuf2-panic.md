<!-- status: draft -->
<!-- suggested title: macOS kernel panic "nfs_vinvalbuf2: ubc_msync failed!, error 22" reached through a fuse-t mount — Apple NFS kext bug (FB filed), not a fuse-t defect -->
...
FB number: FBnnnnnnn (pending) — filed under the macOS File System area, with the three panic reports and the source-level analysis attached. The ask is minimal: route `EINVAL` through the retry/log path `nfs_vinvalbuf2` already has for every other errno, or log and return success as the sibling UPL path has done since Mac OS X 10.5. Until that lands, any NFS mount — fuse-t or network, healthy server or buggy — can hit this; server-side hygiene lowers the frequency but cannot close the kernel's window.
